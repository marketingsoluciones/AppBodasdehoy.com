# Pendientes y estado de Slack con IA (17 feb 2026)
...
**Paneles pendientes (Facturación / Copilot):** Placeholders implementados (planes, cambiar plan, dar crédito admin, uso keys IA). Peticiones a MCP e IA documentadas en **docs/PANELES-PENDIENTES-PETICIONES-MCP-IA.md**. Mensaje listo para Slack en **docs/SLACK-MENSAJE-PANELES-PENDIENTES.md** (copiar/pegar en #copilot-api-ia cuando se quiera pedir ayuda).
...
**Resumen bloqueos:** (1) 503 chat → lo resuelve MCP/IA (credenciales). (2) app-test/chat-test.bodasdehoy.com → lo resolvemos nosotros en Cloudflare (DNS + Public Hostnames al túnel).

**¿Qué es el 503 y qué pasa?** El 503 en POST /webapi/chat/auto significa que IA no pudo devolver una respuesta válida. Suele venir de: **(A) AUTH_ERROR** – la API key del proveedor de IA (OpenAI, Groq, etc.) que usa IA para el whitelabel “bodasdehoy” no es válida o no tiene saldo; esa key la proporciona MCP, no nosotros. **(B) EMPTY_RESPONSE** – el orquestador eligió un provider/modelo pero la respuesta llegó vacía o genérica. IA ya corrigió el tema “provider+modelo incoherente” (usan modelo por defecto); lo que sigue fallando en la mayoría de peticiones es la **autenticación con el proveedor** (credenciales whitelabel bodasdehoy en MCP). Nosotros enviamos X-Development: bodasdehoy y opcionalmente Authorization; el fallo está en el lado de IA/MCP (key inválida o mal configurada).
