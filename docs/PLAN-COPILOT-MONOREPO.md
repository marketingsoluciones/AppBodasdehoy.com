...
Plan paso a paso del monorepo (appEventos + chat-ia), integración del Copilot sin iframe y alineación con el backend (API IA / MCP).

**Resumen ejecutivo**  
✅ Hecho: CopilotEmbed, historial (api-ia si `API_IA_CHAT_HISTORY_URL`; si no, MCP) + fallback en memoria, whitelabel Opción B (api-ia si `API_IA_WHITELABEL_URL`; si no, MCP o `SKIP_WHITELABEL_VIA_MCP`), event_card con botones, eventos SSE, métricas (enganche), tests (chat-history con api-ia y MCP, chat, copilotChat, copilotMetrics), docs (informes api-ia, probar sin navegador).  
...
- `pnpm dev:proxy` — levanta apps principales en local con proxy (recomendado en este repo).
- `pnpm test:web` — tests Jest de `apps/appEventos` (copilotChat + API messages/history) si aplica en tu entorno.
...
| 3.4 | onLoadHistory | Al montar, si hay `sessionId` y `onLoadHistory`, se llama a `getChatHistory(sessionId)` → `GET /api/copilot/chat-history?sessionId=...` → MCP `getChatMessages` → se rellenan los mensajes en el embed. |
...
| 3.8 | api-ia | Procesa el mensaje, llama herramientas si aplica, emite eventos SSE (`event: text`, `event: tool_result`, `event: event_card`, `event: done`, etc.) y al finalizar guarda user + assistant en **MCP** (GraphQL mutation interna). |
...
**Resumen:** Usuario → CopilotEmbed → sendMessage → sendChatMessage → `/api/copilot/chat` → api-ia → SSE → cliente parsea → UI. Historial: MCP (getChatMessages) vía `/api/copilot/chat-history`.

---

## 4. Backend: api-ia y MCP

**Dos servicios distintos.** MCP **no** es api-ia ni apunta a api-ia: es otro backend (GraphQL) donde api-ia **escribe** los mensajes y el front actualmente **lee** el historial.
...
| 5.7 | Comprobar historial | Cerrar y reabrir el panel; si MCP y api-ia guardaron, `onLoadHistory` debería cargar mensajes vía `GET /api/copilot/chat-history`. |
...
| 6.5 | Historial (MCP vía proxy) | Con servidor levantado y token: `curl -s -H "Authorization: Bearer <JWT>" -H "X-Development: bodasdehoy" 'http://127.0.0.1:8080/api/copilot/chat-history?sessionId=user_xxx&limit=50'` → `{"messages":[...]}`. |

---

## 7. Historial de chat (MCP)

| # | Paso | Detalle |
|------|------|---------|
| 7.1 | Quién guarda | **api-ia** guarda en MCP al finalizar el stream (`event: done`). El front **no** debe llamar a `persistChatMessage` ni a `POST /api/chat/messages` para no duplicar. |
| 7.2 | Quién lee | El front llama a `getChatHistory(sessionId)` → `GET /api/copilot/chat-history?sessionId=...` → API route hace proxy a MCP GraphQL `getChatMessages(sessionId, limit)` → devuelve `{ messages: [...] }`. |
...
| 7.5 | Fallback | Si MCP falla o no devuelve datos, `/api/copilot/chat-history` devuelve `{ messages: [] }`. Opcionalmente se puede usar `GET /api/chat/messages` (store en memoria en Next) como fallback leyendo desde esa ruta en lugar de chat-history. |
...
| **docs/ANALISIS-USO-MCP-Y-APIS.md** | Cuándo y por qué se usa MCP vs IA vs otros endpoints en AppBodasdehoy y en LobeChat. |
| **docs/LISTADO-LLAMADAS-MCP-AUDITORIA.md** | **Listado (auditoría)** de llamadas a MCP y referencias legacy “API2”. |
| **docs/INFORME-BACKEND-API-IA-IMPLEMENTAR.md** | **Informe detallado para backend:** qué debe implementar IA (historial, whitelabel/fallback, alcance Copilot) para que el front deje de llamar directo a MCP. |
...
| 3 | Comprobar historial | Cerrar y reabrir el panel del Copilot; si MCP tiene datos para tu `sessionId`, deberían cargarse vía `GET /api/copilot/chat-history`. |
| 4 | Confirmar con backend | Que IA y MCP estén accesibles desde tu entorno (app-test o local con variables correctas: `API_IA_URL`, `API_MCP_GRAPHQL_URL` si aplica). |
...
| 9 | Ajuste si MCP cambia | Si MCP cambia el nombre de la query (p. ej. ya no es `getChatMessages`), actualizar `pages/api/copilot/chat-history.ts` y, si hace falta, el cliente. |
