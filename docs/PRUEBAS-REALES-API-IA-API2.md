# Pruebas reales api-ia y api2 – Cómo generar y enviar evidencia
...
Incluye: chat-test y app-test (GET), api-ia (3 queries reales), api2 (getSubscriptionPlans). Genera `test-results/pruebas-reales-completo-*.json` y `.md`.

---

## Objetivo

Tener **queries reales** contra api-ia y api2, con request/response completos, para **demostrar** si están fallando y poder enviar la evidencia al equipo de api-ia (o api2).
...
```bash
FIREBASE_JWT=<token> node scripts/pruebas-reales-api-ia-api2.mjs
```
...
| **api2** | `POST https://api2.eventosorganizador.com/graphql` | `getSubscriptionPlans(development: "bodasdehoy", is_public: true)` (query real del front) |

Genera en `test-results/`:

- **`pruebas-reales-api-ia-api2-YYYYMMDDHHMMSS.json`** – Request completo (headers, body) y response (status, body preview) de cada llamada. Ideal para adjuntar o pegar en Slack/ticket.
- **`pruebas-reales-api-ia-api2-YYYYMMDDHHMMSS.md`** – Resumen en tabla (OK/FALLO por query).
...
- **api2:** OK – getSubscriptionPlans respondió 200.
...
| `API2_GRAPHQL_URL` | `https://api2.eventosorganizador.com/graphql` | URL GraphQL api2 |
