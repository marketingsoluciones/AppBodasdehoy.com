...
class ChatService {
  ...
  getChatCompletion = async (params: Partial<ChatStreamPayload>, options?: FetchOptions) => {
    ...
    // ✅ CORRECCIÓN: Filtrar original_provider de params ANTES de procesar
    const cleanParams = { ...params };
    delete (cleanParams as any).original_provider;
    delete (cleanParams as any).originalProvider;

    const { provider = ModelProvider.OpenAI, ...res } = cleanParams;
    ...
    // ✅ CORRECCIÓN: Filtrar original_provider de res ANTES del merge
    const cleanRes = { ...res };
    delete (cleanRes as any).original_provider;
    delete (cleanRes as any).originalProvider;
    ...
    // Agregar contexto de usuario desde el store
    ...
    const activeEvent = userEvents.find((e: any) => {
    ...
    if (isValidUser) {
      ...
      if (isEmail) {
        userContextHeaders['X-User-Email'] = currentUserId;
      } else if (isPhone) {
    ...
    // ✅ CORRECCIÓN FINAL: Eliminar original_provider justo antes de serializar (por si se agregó después)
    ...
    payloadString = payloadString.replaceAll(/"originalProvider"\s*:\s*"[^"]*"\s*,?\s*/g, '');
    payloadString = payloadString.replaceAll(/"original_provider"\s*:\s*[^,\]}]+\s*,?\s*/g, '');
    payloadString = payloadString.replaceAll(/"originalProvider"\s*:\s*[^,\]}]+\s*,?\s*/g, '');
