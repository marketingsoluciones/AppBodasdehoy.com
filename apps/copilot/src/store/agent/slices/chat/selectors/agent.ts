...
const currentAgentModel = (s: AgentStoreState): string => {
  ...
  // ✅ Si el provider es "auto" o "auto-max", devolver el valor correspondiente (no el modelo real)
  if (config?.provider === 'auto' || config?.model === 'auto') {
    return 'auto';
  }
  if (config?.provider === 'auto-max' || config?.model === 'auto-max') {
    return 'auto-max';
  }
...
const currentAgentModelProvider = (s: AgentStoreState) => {
  ...
  // ✅ Si el provider es "auto" o "auto-max", devolver el valor correspondiente
  if (config?.provider === 'auto' || config?.model === 'auto') {
    return 'auto';
  }
  if (config?.provider === 'auto-max' || config?.model === 'auto-max') {
