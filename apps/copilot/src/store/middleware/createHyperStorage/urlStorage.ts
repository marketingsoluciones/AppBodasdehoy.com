...
export const creatUrlStorage = <State extends object>(mode: 'hash' | 'search' = 'hash') => {
  ...
  return {
    getItem: <T extends State>(): StorageValue<T> | undefined => {
      ...
      // ✅ FIX: Manejo robusto de parsing JSON desde URL parameters
      const state: any = {};
      for (const [key, value] of searchParameters.entries()) {
        ...
        if (value && (value.startsWith('{') || value.startsWith('['))) {
          try {
            ...
            console.warn(`⚠️ [urlStorage] Error parseando JSON para "${key}", usando valor como string:`, parseError);
      ...
      return { state } as StorageValue<T>;
    },
