...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    while (!state) {
      try {
        ...
        if (error instanceof UserNotFoundError) {
          ...
          // ✅ MODO DESARROLLO: crear usuario automáticamente si auth está desactivado
          else if (!enableClerk) {
            await UserModel.makeSureUserExist(ctx.serverDB, ctx.userId);
            pino.info('create dev user for development mode');
            continue;
          }
        }
    ...
    // ✅ OPTIMIZACIÓN: Ejecutar las 3 queries en paralelo
    const [hasMoreThan4Messages, hasAnyMessages, hasExtraSession] = await Promise.all([
      messageModel.hasMoreThanN(4),
      messageModel.hasMoreThanN(0),
