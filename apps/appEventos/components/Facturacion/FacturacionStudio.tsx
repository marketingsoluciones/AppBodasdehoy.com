...
const FacturacionStudio: FC = () => {
  ...
  return (
    ...
    <div className="hidden md:block" style={{ width: "100%", maxWidth: 1160, margin: "0 auto", padding: "24px 20px 80px", fontFamily: "'Poppins',sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: ".fs-in:focus{border-color:var(--color-primary,var(--color-primary,#EF5B94))!important;}" }} />

      {/* TABS */}
      <div style={{ display: "flex", justifyContent: "center", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        {TABS.map((label, i) => {
          const on = tab === i;
          return <button key={i} onClick={() => setTab(i)} style={{ padding: "9px 18px", borderRadius: 10, border: `1px solid ${on ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#E7E7EA"}`, background: on ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#fff", font: "600 13px Poppins", color: on ? "#fff" : "#6b6b72", whiteSpace: "nowrap", cursor: "pointer" }}>{label}</button>;
        })}
      </div>
      ...
      {curPlan && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 18px", borderRadius: 999, background: "#FCE7F0", border: "1px solid #F8CFE2" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l2.4 4.9 5.4.8-3.9 3.8.9 5.4L12 14.3 7.2 16.9l.9-5.4L4.2 7.7l5.4-.8Z" /></svg>
            <span style={{ font: "600 12px Poppins", color: "var(--color-primary,var(--color-primary,#EF5B94))", whiteSpace: "nowrap" }}>Tu plan actual: <b>{curPlan.name}</b> · {curPrice}</span>
          </div>
        </div>
      )}
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 30 }}>
              <div style={{ display: "inline-flex", background: "#fff", border: "1px solid #ececef", borderRadius: 999, padding: 5, gap: 4 }}>
                <button onClick={() => setAnual(false)} style={{ padding: "9px 22px", borderRadius: 999, background: anual ? "transparent" : "var(--color-primary,var(--color-primary,#EF5B94))", color: anual ? "#6b6b72" : "#fff", font: "600 13px Poppins", boxShadow: anual ? "none" : "0 4px 12px rgba(239,91,148,.3)", cursor: "pointer", border: "none" }}>Mensual</button>
                <button onClick={() => setAnual(true)} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "9px 22px", borderRadius: 999, background: anual ? "var(--color-primary,var(--color-primary,#EF5B94))" : "transparent", color: anual ? "#fff" : "#6b6b72", font: "600 13px Poppins", boxShadow: anual ? "0 4px 12px rgba(239,91,148,.3)" : "none", cursor: "pointer", border: "none" }}>Anual <span style={{ font: "700 12px Poppins", color: anual ? "#fff" : "var(--color-primary,var(--color-primary,#EF5B94))" }}>−20%</span></button>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 18, alignItems: "stretch", marginBottom: 24 }}>
              {mainPlans.map((plan: any) => {
                ...
                return (
                  <div key={plan.plan_id} style={{ position: "relative", background: pop ? "#FDF4F8" : "#fff", border: `${(pop || isCurrent) ? "1.5px" : "1px"} solid ${isCurrent ? "#B7E3CE" : pop ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#ececef"}`, borderRadius: 16, padding: "26px 22px", display: "flex", flexDirection: "column", boxShadow: pop ? "0 10px 30px rgba(239,91,148,.16)" : "none" }}>
                    {pop && <div style={{ position: "absolute", top: -13, left: "50%", transform: "translateX(-50%)", padding: "6px 15px", borderRadius: 999, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "700 11px Poppins", whiteSpace: "nowrap", boxShadow: "0 6px 16px rgba(239,91,148,.3)" }}>★ Más popular</div>}
                    ...
                    <button onClick={() => !isCurrent && openModal(plan)} disabled={isCurrent || subscribing === plan.plan_id} style={{ padding: "12px 20px", borderRadius: 10, background: isCurrent ? "#f7f7f9" : pop ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#fff", border: `1.5px solid ${isCurrent ? "#f7f7f9" : "var(--color-primary,var(--color-primary,#EF5B94))"}`, color: isCurrent ? "#a0a0a8" : pop ? "#fff" : "var(--color-primary,var(--color-primary,#EF5B94))", font: "600 13px Poppins", boxShadow: pop ? "0 6px 16px rgba(239,91,148,.3)" : "none", marginBottom: 20, cursor: isCurrent ? "default" : "pointer" }}>{isCurrent ? "Plan actual" : upgrade ? "Mejorar a este plan" : isFree ? "Cambiar a Gratis" : "Cambiar a este plan"}</button>
                ...
                <div style={{ display: "flex", gap: 30, alignItems: "flex-start", flexWrap: "wrap" }}>
                  <div style={{ flex: 1, minWidth: 260 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,auto)", gap: "10px 34px", justifyContent: "start" }}>
                      {WL_FEATS.map((f) => <div key={f} style={{ display: "flex", alignItems: "center", gap: 9 }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={2.8} strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg><span style={{ font: "500 12.5px Poppins", color: "#6b6b72", whiteSpace: "nowrap" }}>{f}</span></div>)}
                    </div>
                  </div>
                  <div style={{ textAlign: "right", flex: "none" }}>
                    ...
                    {tier === "ENTERPRISE"
                      ...
                      : <button onClick={() => openModal(whitelabelPlan)} style={{ marginTop: 14, padding: "12px 24px", borderRadius: 10, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "600 13px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", border: "none", cursor: "pointer" }}>Cambiar a Whitelabel</button>}
                  </div>
                </div>
              </section>
            )}
              ...
              <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(58,58,66,.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 20 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, padding: "30px 34px", maxWidth: 460, width: "100%", boxShadow: "0 20px 50px rgba(58,58,66,.25)", fontFamily: "'Poppins',sans-serif" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                    ...
                    <button onClick={() => { const p = modal.plan; setModal(null); choose(p.plan_id); }} style={{ padding: "12px 20px", borderRadius: 10, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "600 13px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", border: "none", cursor: "pointer" }}>Confirmar cambio</button>
                  </div>
                </div>
              </div>
            )}
      ...
      {tab === 1 && (
        <div style={{ display: "flex", justifyContent: "center", paddingTop: 16 }}>
          <section style={{ background: "#fff", border: "1px solid #ececef", borderRadius: 16, padding: "44px 50px", maxWidth: 520, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", fontFamily: "'Poppins',sans-serif" }}>
            <div style={{ width: 64, height: 64, borderRadius: "50%", background: "#FCE7F0", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="3" /><path d="M2 10h20M6 15h4" /></svg></div>
            ...
            <button onClick={openPortal} disabled={portalLoading} style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "12px 24px", borderRadius: 10, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "600 13px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", border: "none", cursor: "pointer", opacity: portalLoading ? 0.7 : 1 }}>
