...
const CHECK = (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={2.8} strokeLinecap="round" strokeLinejoin="round" style={{ flex: "none" }}><path d="M20 6L9 17l-5-5" /></svg>
);
const CROSS = (
...
const FacturacionStudioMovil: FC<Props> = ({
  ...
  const wlCard = (current: boolean) => {
    ...
    return (
      <section style={{ background: "#fff", border: "1.5px solid #F8CFE2", borderRadius: 16, padding: "22px 20px", marginBottom: 20 }}>
        ...
        {!current && (
          <button onClick={() => openModal(whitelabelPlan)} style={{ width: "100%", padding: 13, borderRadius: 10, background: "var(--color-primary,var(--color-primary,#EF5B94))", border: "none", color: "#fff", font: "600 13px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", marginTop: 16, cursor: "pointer" }}>Cambiar a Whitelabel</button>
        )}
      </section>
    );
  };
  ...
  return (
    <div className="md:hidden" style={{ maxWidth: 430, margin: "0 auto", padding: "14px 16px 40px", position: "relative", fontFamily: "'Poppins',sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: ".fs-hscroll{scrollbar-width:none;}.fs-hscroll::-webkit-scrollbar{display:none;}.fs-in:focus{border-color:var(--color-primary,var(--color-primary,#EF5B94))!important;}" }} />

      {/* TABS */}
      <div className="fs-hscroll" style={{ display: "flex", gap: 8, overflowX: "auto", padding: "2px 2px 14px" }}>
        {TABS_M.map((label, i) => {
          const on = tab === i;
          return <button key={i} onClick={() => setTab(i)} style={{ flex: "none", padding: "9px 16px", borderRadius: 10, border: `1px solid ${on ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#E7E7EA"}`, background: on ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#fff", font: "600 12.5px Poppins", color: on ? "#fff" : "#6b6b72", whiteSpace: "nowrap", minHeight: 38, cursor: "pointer" }}>{label}</button>;
        })}
      </div>
      ...
      {curPlan && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 15px", borderRadius: 999, background: "#FCE7F0", border: "1px solid #F8CFE2" }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round" style={{ flex: "none" }}><path d="M12 2l2.4 4.9 5.4.8-3.9 3.8.9 5.4L12 14.3 7.2 16.9l.9-5.4L4.2 7.7l5.4-.8Z" /></svg>
            <span style={{ font: "600 11px Poppins", color: "var(--color-primary,var(--color-primary,#EF5B94))" }}>Tu plan: <b>{curPlan.name}</b> · {curPrice}</span>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div style={{ display: "inline-flex", background: "#fff", border: "1px solid #ececef", borderRadius: 999, padding: 5, gap: 4 }}>
              <button onClick={() => setAnual(false)} style={{ padding: "10px 20px", borderRadius: 999, background: anual ? "transparent" : "var(--color-primary,var(--color-primary,#EF5B94))", color: anual ? "#6b6b72" : "#fff", font: "600 12.5px Poppins", boxShadow: anual ? "none" : "0 4px 12px rgba(239,91,148,.3)", border: "none", minHeight: 40, cursor: "pointer" }}>Mensual</button>
              <button onClick={() => setAnual(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 20px", borderRadius: 999, background: anual ? "var(--color-primary,var(--color-primary,#EF5B94))" : "transparent", color: anual ? "#fff" : "#6b6b72", font: "600 12.5px Poppins", boxShadow: anual ? "0 4px 12px rgba(239,91,148,.3)" : "none", border: "none", minHeight: 40, cursor: "pointer" }}>Anual <span style={{ font: "700 11.5px Poppins", color: anual ? "#fff" : "var(--color-primary,var(--color-primary,#EF5B94))" }}>−20%</span></button>
            </div>
          </div>
          ...
          <div ref={sliderRef} onScroll={onSliderScroll} className="fs-hscroll" style={{ display: "flex", gap: 14, overflowX: "auto", scrollSnapType: "x mandatory", padding: "14px 2px 4px", margin: "0 -16px 6px" }}>
            ...
            {mainPlans.map((plan: any) => {
              ...
              return (
                <div key={plan.plan_id} style={{ position: "relative", flex: "none", width: "82%", scrollSnapAlign: "center", background: pop ? "#FDF4F8" : "#fff", border: `${(pop || isCurrent) ? "1.5px" : "1px"} solid ${isCurrent ? "#B7E3CE" : pop ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#ececef"}`, borderRadius: 16, padding: "24px 20px 20px", boxShadow: pop ? "0 10px 30px rgba(239,91,148,.16)" : "none" }}>
                  {pop && <div style={{ position: "absolute", top: -12, left: 20, padding: "5px 13px", borderRadius: 999, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "700 10.5px Poppins", whiteSpace: "nowrap", boxShadow: "0 6px 16px rgba(239,91,148,.3)" }}>★ Más popular</div>}
                  ...
                  <button onClick={() => !isCurrent && openModal(plan)} disabled={isCurrent || subscribing === plan.plan_id} style={{ width: "100%", padding: 13, borderRadius: 10, background: isCurrent ? "#f7f7f9" : pop ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#fff", border: `1.5px solid ${isCurrent ? "#f7f7f9" : "var(--color-primary,var(--color-primary,#EF5B94))"}`, color: isCurrent ? "#a0a0a8" : pop ? "#fff" : "var(--color-primary,var(--color-primary,#EF5B94))", font: "600 13px Poppins", boxShadow: pop ? "0 6px 16px rgba(239,91,148,.3)" : "none", margin: "14px 0 16px", minHeight: 46, cursor: isCurrent ? "default" : "pointer" }}>{cta}</button>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 7, marginBottom: 20 }}>
            {mainPlans.map((_: any, i: number) => <span key={i} style={{ width: i === dot ? 20 : 7, height: 7, borderRadius: 999, background: i === dot ? "var(--color-primary,var(--color-primary,#EF5B94))" : "#e3d9de", transition: "all .25s" }} />)}
          </div>
      ...
      {tab === 1 && (
        <section style={{ background: "#fff", border: "1px solid #ececef", borderRadius: 16, padding: "36px 24px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
          <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#FCE7F0", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18 }}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,var(--color-primary,#EF5B94))" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="3" /><path d="M2 10h20M6 15h4" /></svg></div>
          ...
          <button onClick={openPortal} disabled={portalLoading} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", padding: 13, borderRadius: 10, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "600 13px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", border: "none", minHeight: 46, cursor: "pointer", opacity: portalLoading ? 0.7 : 1 }}>
        ...
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(58,58,66,.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 9999 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: "20px 20px 0 0", padding: "26px 22px 34px", width: "100%", maxWidth: 430, boxShadow: "0 -10px 40px rgba(58,58,66,.25)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <button onClick={() => { const p = modal.plan; setModal(null); choose(p.plan_id); }} style={{ width: "100%", padding: 14, borderRadius: 12, background: "var(--color-primary,var(--color-primary,#EF5B94))", color: "#fff", font: "600 13.5px Poppins", boxShadow: "0 6px 16px rgba(239,91,148,.3)", border: "none", minHeight: 48, cursor: "pointer" }}>Confirmar cambio</button>
