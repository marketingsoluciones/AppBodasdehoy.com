...
const ChatToggleButton: FC<ChatToggleButtonProps> = ({ className = '', studio = false }) => {
  ...
  if (studio) {
    const sparkle = (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="var(--color-primary,var(--color-primary,#EF5B94))"><path d="M12 3l1.7 4.6L18 9l-4.3 1.4L12 15l-1.7-4.6L6 9l4.3-1.4L12 3z" /><path d="M18.5 14l.9 2.3 2.3.9-2.3.9-.9 2.3-.9-2.3-2.3-.9 2.3-.9.9-2.3z" opacity=".6" /></svg>
    );
    return (
      <>
        ...
        >
          {sparkle}
          <span style={{ position: 'absolute', top: 2, right: 2, width: 8, height: 8, borderRadius: '50%', background: isOpen ? 'var(--color-primary,var(--color-primary,#EF5B94))' : '#2FB37E', border: '2px solid #fff' }} />
        </button>
        {/* Desktop: píldora */}
        <button
          type="button"
        ...
        >
          {sparkle}
          <span style={{ font: '600 13px Poppins', color: 'var(--color-primary,var(--color-primary,#EF5B94))' }}>Copilot</span>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: isOpen ? 'var(--color-primary,var(--color-primary,#EF5B94))' : '#2FB37E' }} />
        </button>
      </>
    );
  }
