...
// (var(--color-primary,#EF5B94)) con subrayado rosa 2.5px; las inactivas en gris #9a9aa2 sin subrayado.
        ...
        return (
          <button
            ...
            className={`flex-1 min-w-0 flex items-center justify-center border-b-[2.5px] -mb-px transition-colors
              ${active ? "border-[var(--color-primary,#EF5B94)]" : "border-transparent"}
              ${elem?.title === "invitados" ? "md:hidden" : ""}`}
          >
            <span className={`text-[10.5px] font-semibold capitalize leading-none truncate max-w-full ${active ? "text-[var(--color-primary,#EF5B94)]" : "text-[#9a9aa2]"}`}>{t(elem?.title)}</span>
          </button>
        )
      })}
    </div>
