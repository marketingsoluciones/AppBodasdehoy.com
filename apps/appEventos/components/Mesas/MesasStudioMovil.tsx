    ...
    <div className="md:hidden flex flex-col" style={{ background: "#fff", minHeight: "100%", position: "relative", fontFamily: "'Poppins',sans-serif" }}>
      ...
      <div style={{ background: "#fff", padding: "12px 18px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f2f2f4" }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ font: "700 10px Poppins", marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}><span style={{ color: "var(--color-primary,#EF5B94)" }}>{(event?.tipo || "EVENTO").toUpperCase()}</span><span style={{ color: "#9aa2ab", fontWeight: 600 }}> · {event?.nombre}</span></div>
        </div>
      ...
      <div style={{ background: "#f1f1f4", flex: 1, paddingBottom: 28 }}>
        ...
        <div className="msm-hs" style={{ display: "flex", gap: 8, overflowX: "auto", padding: "14px 16px 12px" }}>
          {spaces.map((p) => {
            ...
            return (
              <div key={p._id} onClick={() => { setPlanoId(p._id); setPicked(null); setSelected(null); }} style={{ flex: "none", display: "flex", alignItems: "center", gap: 7, padding: "8px 14px", borderRadius: 999, cursor: "pointer", background: on ? "var(--color-primary,#EF5B94)" : "#fff", border: `1.5px solid ${on ? "var(--color-primary,#EF5B94)" : "#f0f0f2"}` }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={on ? "#fff" : "var(--color-primary,#EF5B94)"} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="1.5" /><path d="M3 13h5M12 13h9M15 3v4M15 11v10" /></svg>
                <span style={{ font: "600 12px Poppins", color: on ? "#fff" : "var(--color-primary,#EF5B94)", whiteSpace: "nowrap" }}>{p.title}</span>
          ...
          <div onClick={() => { setNewPlanoName(""); setNewPlanoOpen(true); }} style={{ flex: "none", display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 999, cursor: "pointer", background: "#fff", border: "1.5px dashed #f0aecb", color: "var(--color-primary,#EF5B94)", font: "600 12px Poppins", whiteSpace: "nowrap" }}>＋ Plano</div>
        </div>

        {/* Banner "sentar a X" */}
        {picked && (
          <div style={{ position: "sticky", top: 8, zIndex: 25, margin: "0 12px 10px", display: "flex", alignItems: "center", gap: 8, background: "var(--color-primary,#EF5B94)", borderRadius: 999, padding: "10px 16px", boxShadow: "0 6px 16px rgba(239,91,148,.4)", animation: "msm-fade .2s ease" }}>
          ...
          <div onClick={() => setSelected(null)} style={{ position: "relative", height: 330, borderRadius: 18, background: "#F3F1EC", overflow: "hidden", boxShadow: "0 6px 20px rgba(0,0,0,.07)" }}>
            ...
            <div style={{ position: "absolute", top: 10, left: 10, display: "flex", alignItems: "center", gap: 8, zIndex: 3 }}>
              <div style={{ display: "flex", alignItems: "center", background: "#fff", borderRadius: 10, boxShadow: "0 3px 10px rgba(0,0,0,.08)", overflow: "hidden" }}>
                <button onClick={(e) => { e.stopPropagation(); setZoom((z) => Math.max(50, z - 10)); }} style={{ width: 32, height: 32, color: "var(--color-primary,#EF5B94)", fontSize: 16, border: "none", background: "none", cursor: "pointer" }}>−</button>
                <div style={{ font: "700 11px Poppins", color: "#3A3A42", padding: "0 4px", minWidth: 40, textAlign: "center" }}>{zoom}%</div>
                <button onClick={(e) => { e.stopPropagation(); setZoom((z) => Math.min(150, z + 10)); }} style={{ width: 32, height: 32, color: "var(--color-primary,#EF5B94)", fontSize: 16, border: "none", background: "none", cursor: "pointer" }}>＋</button>
              </div>
              <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 3px 10px rgba(0,0,0,.08)", padding: "8px 12px", font: "600 11px Poppins", color: "var(--color-primary,#EF5B94)", whiteSpace: "nowrap" }}>{space?.title}</div>
            </div>

            {/* Bloquear / Editar plano (mover mesas y muebles) */}
            {!canvasEmpty && (
              <div onClick={(e) => { e.stopPropagation(); toggleLock(); }} style={{ position: "absolute", top: 10, right: 10, zIndex: 3, display: "flex", alignItems: "center", gap: 6, background: locked ? "#fff" : "var(--color-primary,#EF5B94)", color: locked ? "#3A3A42" : "#fff", borderRadius: 10, boxShadow: "0 3px 10px rgba(0,0,0,.1)", padding: "8px 11px", font: "600 10.5px Poppins", cursor: "pointer" }}>
                {locked
                  ? <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,#EF5B94)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="9" rx="2" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg>
              ...
              {elements.map((el, i) => {
                ...
                return (
                  <div key={el._id}
                    ...
                    {on && (
                      <div onClick={(e) => { e.stopPropagation(); deleteElement(el); }} onPointerDown={(e) => e.stopPropagation()} style={{ position: "absolute", top: -13, left: -13, width: 26, height: 26, borderRadius: "50%", background: "#fff", border: "1px solid #f2c9d9", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 3px 8px rgba(0,0,0,.18)", zIndex: 6 }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary,#EF5B94)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M8 6V4h8v2M6 6l1 14h10l1-14" /></svg></div>
                    )}
                    <div style={{ width: w, height: h, display: "flex", alignItems: "center", justifyContent: "center", color: "#6b6b72", border: on ? "2px dashed var(--color-primary,#EF5B94)" : "none", borderRadius: 10 }}>{furnIcon(el.tipo || el.title, Math.min(w, h))}</div>
                  </div>
                );
              })}
              {/* Mesas */}
                ...
                return (
                  <div key={t._id}
                    ...
                    {slots.map((s) => {
                      ...
                      return <div key={s.chair} style={{ position: "absolute", left: s.x, top: s.y, width: 15, height: 15, borderRadius: "50%", background: seatOn ? "var(--color-primary,#EF5B94)" : "#fff", border: `1.5px solid ${seatOn ? "var(--color-primary,#EF5B94)" : "#cfcfd6"}`, zIndex: 1, boxShadow: seatOn ? "0 0 0 2px rgba(239,91,148,.18)" : "none" }} />;
                    })}
                    ...
                    <div style={{ position: "relative", width: g.w, height: g.h, borderRadius: g.round ? "50%" : 12, background: "#F0F0F2", border: `2.5px solid ${on ? "var(--color-primary,#EF5B94)" : picked ? "var(--color-primary,#EF5B94)" : "#d8d8de"}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 10px rgba(0,0,0,.1)" }}>
                      <span style={{ font: "700 13px Poppins", color: "#6b6b72" }}>{t.title}</span>
                      {seatedCount > 0 && <div style={{ position: "absolute", top: -6, right: -6, minWidth: 18, height: 18, padding: "0 4px", borderRadius: 10, background: "var(--color-primary,#EF5B94)", color: "#fff", font: "700 9px Poppins", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid #fff", boxShadow: "0 2px 5px rgba(0,0,0,.2)" }}>{seatedCount}</div>}
                    </div>
                  </div>
                );
              })}
            ...
            {canvasEmpty && (
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, zIndex: 2, padding: 20, textAlign: "center" }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#fff", border: "2px dashed #f0aecb", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--color-primary,#EF5B94)", boxShadow: "0 6px 18px rgba(0,0,0,.06)" }}><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6}><ellipse cx="12" cy="9" rx="8" ry="3" /><path d="M6 10v8M18 10v8" /></svg></div>
                ...
                <button onClick={(e) => { e.stopPropagation(); openCreateTable(); }} style={{ padding: "11px 18px", borderRadius: 10, background: "var(--color-primary,#EF5B94)", color: "#fff", font: "600 12px Poppins", border: "none", boxShadow: "0 8px 20px rgba(239,91,148,.35)", cursor: "pointer" }}>＋ Crea tu primera mesa</button>
              </div>
            )}

            {/* FAB "Añadir mesa" cuando hay contenido */}
            {!canvasEmpty && (
              <button onClick={(e) => { e.stopPropagation(); openCreateTable(); }} style={{ position: "absolute", right: 12, bottom: 12, zIndex: 3, padding: "12px 17px", borderRadius: 10, background: "var(--color-primary,#EF5B94)", color: "#fff", font: "600 12.5px Poppins", border: "none", whiteSpace: "nowrap", boxShadow: "0 8px 20px rgba(239,91,148,.4)", cursor: "pointer" }}>＋ Añadir mesa</button>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", background: "#fff", borderRadius: 14, margin: "14px 12px 0", padding: 5, gap: 3, boxShadow: "0 3px 12px rgba(0,0,0,.05)" }}>
          {([["mesas", "Mesas"], ["mobiliario", "Mobiliario"], ["resumen", "Resumen"]] as const).map(([k, label]) => {
            const on = tab === k;
            return <div key={k} onClick={() => setTab(k)} style={{ flex: 1, textAlign: "center", padding: "9px 2px", borderRadius: 10, font: "600 11.5px Poppins", cursor: "pointer", background: on ? "var(--color-primary,#EF5B94)" : "transparent", color: on ? "#fff" : "#8a8a90" }}>{label}</div>;
          })}
        </div>
        ...
        <div style={{ padding: "13px 12px 0" }}>
          ...
          {tab === "mesas" && (
            <div style={{ animation: "msm-fade .2s ease", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ font: "700 10.5px Poppins", color: "#b3b3ba", letterSpacing: 1, textTransform: "uppercase", margin: "0 4px" }}>Mesas en «{space?.title}» · <span style={{ color: "var(--color-primary,#EF5B94)" }}>{tables.length}</span></div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 9 }}>
                {MOBILIARIO.map((f) => (
                  <div key={f.model} onClick={() => addFurniture(f)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, padding: "12px 4px", borderRadius: 12, background: "#fff", border: "1.5px solid #f0f0f2", cursor: "pointer" }}>
                    <div style={{ color: "var(--color-primary,#EF5B94)" }}>{(() => { const Icon = f.Icon; return <Icon width={22} height={22} />; })()}</div>
            ...
            <div style={{ animation: "msm-fade .2s ease" }}>
              <div style={{ borderRadius: 13, padding: 14, background: "#fff", border: "1px solid #f0f0f2", boxShadow: "0 3px 10px rgba(0,0,0,.04)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}><div style={{ font: "700 24px Poppins", lineHeight: 1, color: "var(--color-primary,#EF5B94)" }}>{resumenPct}%</div><div style={{ font: "600 10.5px Poppins", color: "#8a8a90" }}>ocupado</div></div>
          ...
          <div style={{ background: "#fff", borderRadius: 15, marginTop: 14, padding: "13px 14px", boxShadow: "0 3px 12px rgba(0,0,0,.05)" }}>
            <div onClick={() => setGuestsOpen((v) => !v)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
              ...
              <div style={{ font: "600 10.5px Poppins", color: "#a0a0a8" }}>Sentados <span style={{ color: "var(--color-primary,#EF5B94)" }}>{seatedInSpace.size}/{invitados.length}</span></div>
            </div>
            {guestsOpen && (
              <div style={{ animation: "msm-fade .2s ease", marginTop: 11 }}>
                <div style={{ display: "flex", background: "#f2f2f4", borderRadius: 9, padding: 3, gap: 2, marginBottom: 10 }}>
                  {([["todos", "Todos"], ["porsentar", "Por sentar"], ["sentados", "Sentados"]] as const).map(([k, label]) => {
                    const on = gFilter === k;
                    return <div key={k} onClick={() => setGFilter(k)} style={{ flex: 1, textAlign: "center", padding: "7px 2px", borderRadius: 7, font: "600 10.5px Poppins", cursor: "pointer", background: on ? "#fff" : "transparent", color: on ? "var(--color-primary,#EF5B94)" : "#8a8a90", boxShadow: on ? "0 1px 3px rgba(0,0,0,.08)" : "none" }}>{label}</div>;
                  })}
                </div>
                ...
                <div className="msm-hs" style={{ display: "flex", flexDirection: "column", gap: 7, maxHeight: 260, overflow: "auto" }}>
                  ...
                  {listForFilter.map((gm) => {
                    ...
                    if (info) {
                      return (
                        <div key={gm._id} style={{ display: "flex", alignItems: "center", gap: 10, padding: 9, borderRadius: 11, background: "#fff", border: "1.5px solid #f0f0f2" }}>
                          ...
                          <div style={{ font: "600 10px Poppins", color: "var(--color-primary,#EF5B94)", background: "#FCE7F0", padding: "3px 8px", borderRadius: 8, whiteSpace: "nowrap" }}>{info.table.title}</div>
                    ...
                    return (
                      <div key={gm._id} onClick={() => setPicked(isPicked ? null : gm)} style={{ display: "flex", alignItems: "center", gap: 10, padding: 9, borderRadius: 11, cursor: "pointer", background: isPicked ? "#FCE7F0" : "#faf9fb", border: `1.5px solid ${isPicked ? "var(--color-primary,#EF5B94)" : "#f0f0f2"}` }}>
                        <div style={{ width: 28, height: 28, borderRadius: "50%", flex: "none", background: "linear-gradient(135deg,#c4c4cc,#a8a8b0)" }} />
                        <div style={{ flex: 1, minWidth: 0, font: "600 12.5px Poppins", color: "#3A3A42", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{gm.nombre}</div>
                        <div style={{ font: "600 10px Poppins", color: isPicked ? "var(--color-primary,#EF5B94)" : "#a0a0a8" }}>{isPicked ? "Elige mesa…" : "Sentar"}</div>
                      </div>
                    );
                  })}
                </div>
                <button onClick={() => setAddOpen(true)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: 10, borderRadius: 11, background: "#fff", border: "1.5px dashed #f0aecb", color: "var(--color-primary,#EF5B94)", font: "600 12.5px Poppins", marginTop: 10, cursor: "pointer" }}><span style={{ fontSize: 15 }}>＋</span>Añadir Invitados</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="msm-hs" style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 420, background: "#fff", borderRadius: "22px 22px 0 0", zIndex: 99999, boxShadow: "0 -12px 40px rgba(0,0,0,.2)", animation: "msm-up .25s ease", maxHeight: "86vh", overflow: "auto", fontFamily: "'Poppins',sans-serif" }}>
            ...
            <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                  {SHAPE_LABELS.map((sh) => {
                    const on = dShape === sh;
                    return <div key={sh} onClick={() => setDShape(sh)} style={{ padding: "8px 13px", borderRadius: 9, font: "600 12px Poppins", cursor: "pointer", border: `1.5px solid ${on ? "var(--color-primary,#EF5B94)" : "#E7E7EA"}`, background: on ? "#FCE7F0" : "#fff", color: on ? "var(--color-primary,#EF5B94)" : "#6b6b72" }}>{sh}</div>;
                  })}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <button onClick={() => setDSillas((n) => Math.max(1, n - 1))} style={{ width: 40, height: 40, borderRadius: 10, background: "#f7f7f9", color: "var(--color-primary,#EF5B94)", fontSize: 20, border: "none", cursor: "pointer" }}>−</button>
                  <div style={{ flex: 1, font: "700 19px Poppins", color: "#3A3A42", textAlign: "center" }}>{dSillas}</div>
                  <button onClick={() => setDSillas((n) => Math.min(40, n + 1))} style={{ width: 40, height: 40, borderRadius: 10, background: "#f7f7f9", color: "var(--color-primary,#EF5B94)", fontSize: 20, border: "none", cursor: "pointer" }}>＋</button>
                </div>
              </div>
              {/* preview */}
            ...
            <div style={{ padding: "12px 18px 20px", borderTop: "1px solid #f0f0f2", display: "flex", gap: 10 }}>
              ...
              <button onClick={confirmDesign} disabled={busy} style={{ flex: 1, padding: 13, borderRadius: 11, background: busy ? "#f4b8d1" : "var(--color-primary,#EF5B94)", color: "#fff", font: "600 13px Poppins", border: "none", boxShadow: "0 6px 16px rgba(239,91,148,.32)", cursor: busy ? "default" : "pointer" }}>{editId ? "Guardar cambios" : "Crear mesa"}</button>
            </div>
          </div>
        </div>,
        document.body,
        ...
        <div onClick={() => setNewPlanoOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(43,43,48,.38)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'Poppins',sans-serif" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 380, background: "#fff", borderRadius: 18, boxShadow: "0 24px 60px rgba(0,0,0,.28)", padding: 22 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
              ...
              <button onClick={createPlano} disabled={busy || !newPlanoName.trim()} style={{ flex: 1, padding: 12, borderRadius: 11, background: newPlanoName.trim() && !busy ? "var(--color-primary,#EF5B94)" : "#f4b8d1", color: "#fff", font: "600 12.5px Poppins", border: "none", boxShadow: "0 6px 16px rgba(239,91,148,.28)", cursor: newPlanoName.trim() && !busy ? "pointer" : "default" }}>Crear plano</button>
            </div>
          </div>
        </div>,
        document.body,
