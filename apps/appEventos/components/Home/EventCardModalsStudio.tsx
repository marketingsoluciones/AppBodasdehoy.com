  ...
  return createPortal(
    <div onClick={onCancel} style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(40,40,46,.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'Poppins',sans-serif" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 380, maxWidth: "92vw", background: "#fff", borderRadius: 20, boxShadow: "0 30px 80px rgba(0,0,0,.3)", padding: "28px 28px 24px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
        <span style={{ width: 52, height: 52, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16, background: esBorrar ? "#FBE3ED" : "#FCE7F0", color: esBorrar ? "#D83E7C" : "var(--color-primary,#EF5B94)" }}>{icon}</span>
        ...
        <div style={{ font: "400 12.5px/1.6 Poppins", color: "#8a8a90", marginBottom: 20 }}>
          {esBorrar
            ...
            : <>{t("Se moverá a", { defaultValue: "Se moverá a" })} <span style={{ color: "var(--color-primary,#EF5B94)", fontWeight: 600 }}>{t("Archivados", { defaultValue: "Archivados" })}</span> {t("y podrás recuperarlo en cualquier momento.", { defaultValue: "y podrás recuperarlo en cualquier momento." })}</>}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, width: "100%" }}>
          ...
          <button onClick={onConfirm} style={{ flex: 1, padding: 12, borderRadius: 12, border: "none", color: "#fff", font: "600 13px Poppins", cursor: "pointer", background: esBorrar ? "#D83E7C" : "var(--color-primary,#EF5B94)", boxShadow: esBorrar ? "0 5px 14px rgba(216,62,124,.3)" : "0 5px 14px rgba(239,91,148,.28)" }}>{esBorrar ? t("Borrar") : t("Archivar", { defaultValue: "Archivar" })}</button>
        </div>
      </div>
    </div>,
    document.body
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 440, maxWidth: "94vw", maxHeight: "90vh", background: "#fff", borderRadius: 20, boxShadow: "0 30px 80px rgba(0,0,0,.3)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "18px 26px", display: "flex", flexDirection: "column", gap: 16, overflow: "auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={() => !saving && invitar()} disabled={saving} style={{ padding: "11px 18px", borderRadius: 10, background: "var(--color-primary,#EF5B94)", color: "#fff", font: "600 12.5px Poppins", border: "none", cursor: saving ? "default" : "pointer", whiteSpace: "nowrap", boxShadow: "0 4px 12px rgba(239,91,148,.25)", opacity: saving ? 0.7 : 1 }}>{t("Invitar", { defaultValue: "Invitar" })}</button>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 10, background: "#fafafa" }}>
              <span style={{ width: 30, height: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", font: "700 12px Poppins", flex: "none", background: "var(--color-primary,#EF5B94)", color: "#fff" }}>{(ownerName || "?").charAt(0).toUpperCase()}</span>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button className="msc-copiar" onClick={copiar} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 15px", borderRadius: 10, background: "#fff", border: "1.5px solid #E7E7EA", color: "var(--color-primary,#EF5B94)", font: "600 12px Poppins", cursor: "pointer", whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", padding: "14px 26px 18px", borderTop: "1px solid #f0f0f2" }}>
          <button onClick={onClose} style={{ padding: "11px 26px", borderRadius: 11, background: "var(--color-primary,#EF5B94)", color: "#fff", font: "600 13px Poppins", border: "none", cursor: "pointer", boxShadow: "0 5px 14px rgba(239,91,148,.28)" }}>{t("Hecho", { defaultValue: "Hecho" })}</button>
        </div>
        <style dangerouslySetInnerHTML={{ __html: ".msc-x:hover{background:#f5f5f7;}.msc-persona:hover{background:#fdf7fa;}.msc-quitar:hover{background:#FBE3ED;color:#D83E7C;}.msc-copiar:hover{border-color:var(--color-primary,#EF5B94);}" }} />
      </div>
    </div>,
    document.body
  );
