...
import { shouldIgnoreFile } from './blackList';
...
export const loadFile = async (
  ...
  // Check if file should be ignored (macOS metadata files, etc.)
  const filename = path.basename(filePath);
  const baseFilename = filename;
  const finalFilename = fileMetadata?.filename ?? baseFilename;

  if (shouldIgnoreFile(filename)) {
    log('File should be ignored, returning empty document:', filename);
    return {
      content: '',
      ...
      metadata: {
        error: 'File is a system metadata file and should be ignored'
      },
