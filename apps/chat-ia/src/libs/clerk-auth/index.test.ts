...
describe('ClerkAuth', () => {
  ...
  describe('getAuthFromRequest', () => {
    it('should get auth from request and return original user ID when no mapping', async () => {
      // 设置模拟返回值
      vi.mocked(getAuth).mockReturnValue({ userId: 'original_user_id' } as any);

      const clerkAuth = new ClerkAuth();
      const mockRequest = {} as NextRequest;
      const result = await clerkAuth.getAuthFromRequest(mockRequest);
    ...
    it('should map user ID in development environment', async () => {
      ...
      const clerkAuth = new ClerkAuth();
      const result = await clerkAuth.getAuthFromRequest({} as NextRequest);
