...
export function LobeNextAuthDbAdapter(): Adapter {
  // ✅ MEJORA: En desarrollo, usar valor por defecto si APP_URL no está disponible
  let baseUrl = appEnv.APP_URL;
  if (!baseUrl && (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV)) {
    console.warn('⚠️ LobeNextAuthDbAdapter: APP_URL no está configurada, usando http://localhost:8000');
    baseUrl = 'http://localhost:8000';
  }

  // En producción, lanzar error si APP_URL no está disponible
  if (!baseUrl) {
