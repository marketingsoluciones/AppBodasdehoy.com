      ...
      next(op).subscribe({
        ...
        error: async (err) => {
          ...
          if (showError) {
            ...
            switch (status) {
              case 401: {
                // ✅ NO mostrar login en modo desarrollo o para file manager
                const { enableAuth } = await import('@/const/auth');
                const isFileManagerRoute = typeof window !== 'undefined' && window.location.pathname.includes('/files');

                // Solo redirigir si auth está activado Y no es file manager
                if (enableAuth && !isFileManagerRoute) {
                  loginRequired.redirect();
                } else {
                  console.warn('⚠️ Error 401 ignorado (auth desactivado o file manager)', { enableAuth, isFileManagerRoute });
                }
                break;
              }
