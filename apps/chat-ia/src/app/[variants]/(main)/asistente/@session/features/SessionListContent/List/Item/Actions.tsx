...
const Actions = memo<ActionProps>(({ group, id, openCreateGroupModal, setOpen }) => {
  ...
  const [pin, removeSession, pinSession, duplicateSession, updateSessionGroup, renameSession] =
    useSessionStore((s) => {
        ...
        [
          ...
          },
          {
            ...
            onClick: ({ domEvent }) => {
              ...
              const session = sessionSelectors.getSessionById(id)(useSessionStore.getState());
              const currentTitle = session.meta?.title || '';
              ...
              modal.confirm({
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        if (input) {
                          // @ts-expect-error - renameSession signature
                          renameSession(id, { title: input.value });
                          // @ts-expect-error - i18n namespace
                          message.success(t('renameSuccess', { ns: 'common' }));
                        }
                        // @ts-expect-error - modal.destroyAll
                        modal.destroyAll();
                      }
                    }}
                    style={{
                ...
                onOk: async () => {
                  const input = document.getElementById('session-rename-input') as HTMLInputElement;
                  if (input) {
                    // @ts-expect-error - renameSession signature
                    await renameSession(id, { title: input.value });
                    // @ts-expect-error - i18n namespace
                    message.success(t('renameSuccess', { ns: 'common' }));
                  }
                },
