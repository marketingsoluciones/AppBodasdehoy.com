...
export default function McpDebugPage() {
  ...
  const testAll = async () => {
    ...
    for (const development of developments) {
      ...
      setResults((prev) => [...prev, { development, status: 'pending', supportKey }]);

      const result = await testWhitelabel(development, supportKey);

      testResults.push(result);
      setResults((prev) => prev.map((r) => (r.development === development ? result : r)));
    }
  ...
  return (
    <Flexbox padding={24} style={{ margin: '0 auto', maxWidth: 1400 }}>
      <Flexbox gap={16} style={{ marginBottom: 24 }}>
        <Title level={2}>🔍 MCP Debug - Test de Whitelabels</Title>
        <Paragraph>Esta herramienta prueba la conexión con API MCP usando los supportKeys de cada whitelabel.</Paragraph>
        <Button disabled={testing} loading={testing} onClick={testAll} size="large" type="primary">
          ...
          <Card
            ...
            style={{
              borderLeft: `4px solid ${
                result.status === 'success' ? '#52c41a' : result.status === 'error' ? '#ff4d4f' : '#1890ff'
              }`,
            }}
          >
            <Flexbox gap={12}>
                ...
                >
                  ...
                  {result.response.info && (
                    <Flexbox gap={8} style={{ background: '#fff', borderRadius: 4, padding: 12 }}>
                      ...
                      <Flexbox gap={4}>
                        <Text>
                          <strong>Nombre:</strong> {result.response.info.name || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Descripción:</strong> {result.response.info.description || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Dominio:</strong> {result.response.info.domain || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Email:</strong> {result.response.info.contactEmail || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Activo:</strong> {result.response.info.active ? '✅ Sí' : '❌ No'}
                        </Text>
                      </Flexbox>
                    </Flexbox>
                  )}
                  ...
                  <Flexbox gap={8} style={{ background: '#fff', borderRadius: 4, padding: 12 }}>
                    ...
                    <Flexbox gap={4}>
                      <Text>
                        <strong>Provider:</strong> <Tag color="blue">{result.response.aiProvider || 'Sin configurar'}</Tag>
                      </Text>
                      <Text>
                        <strong>Model:</strong> <Tag color="green">{result.response.aiModel || 'Sin configurar'}</Tag>
                      </Text>
                      <Text>
                      ...
                      {result.response.availableProviders && (
                        <Text>
                          <strong>Providers disponibles:</strong> {result.response.availableProviders.join(', ') || 'N/A'}
                        </Text>
                      )}
                    </Flexbox>
                  </Flexbox>
                  ...
                  {result.response.messages && (
                    <Flexbox gap={8} style={{ background: '#fff', borderRadius: 4, padding: 12 }}>
                      ...
                      <Flexbox gap={4}>
                        <Text>
                          <strong>Asistente:</strong> {result.response.messages.assistantName || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Título bienvenida:</strong> {result.response.messages.welcomeTitle || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Subtítulo:</strong> {result.response.messages.welcomeSubtitle || 'N/A'}
                        </Text>
                        <Text>
                          <strong>Placeholder:</strong> {result.response.messages.inputPlaceholder || 'N/A'}
                        </Text>
                      </Flexbox>
                    </Flexbox>
                  )}
                    ...
                    <Flexbox gap={8} style={{ background: '#fff', borderRadius: 4, padding: 12 }}>
                      ...
                      <Flexbox gap={4}>
                        {result.response.branding.logo && (
                          <Text>
                            <strong>Logo:</strong> {result.response.branding.logo}
                          </Text>
                        )}
                  ...
                  <details style={{ marginTop: 8 }}>
                    <summary style={{ color: '#1890ff', cursor: 'pointer', fontWeight: 500 }}>📄 Ver JSON completo</summary>
                    <pre
                      style={{
                        background: '#f5f5f5',
