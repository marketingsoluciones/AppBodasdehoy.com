...
const TitleTags = memo(() => {
  ...
  return isLoading && isLogin ? (
    ...
    <Flexbox align={'center'} gap={4} horizontal>
      <ModelSwitchPanel>
        {/* ✅ Mostrar "Auto" o "Auto Max" cuando provider es auto, no el modelo real */}
        {provider === 'auto' || model === 'auto' ? (
          <Tag color="green" style={{ alignItems: 'center', display: 'flex', gap: 4 }}>
            <Infinity size={14} />
            Auto
          </Tag>
        ) : provider === 'auto-max' || model === 'auto-max' ? (
          <Tag color="blue" style={{ alignItems: 'center', display: 'flex', gap: 4 }}>
            <Infinity size={14} />
            Auto Max
          </Tag>
        ) : (
