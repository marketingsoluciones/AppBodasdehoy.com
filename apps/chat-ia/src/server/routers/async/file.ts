    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const embeddingPromise = async () => {
          ...
          const {
            shouldUseMiddleware,
            batchGenerateEmbeddingsViaMiddleware,
          ...
          try {
            if (useMiddleware) {
              ...
              console.log(`🔧 Usando middleware para embeddings (Ollama + ChromaDB)`);
              
              await pMap(
                requestArray,
                async (chunks, index) => {
                  console.log(`run embedding task ${index + 1} (middleware)`);

                  const texts = chunks.map((c) => c.text);
                  const chunkIds = chunks.map((c) => c.id);

                  // Generar embeddings en lote via middleware
                  await batchGenerateEmbeddingsViaMiddleware(
                    texts,
                    ctx.userId,
                    input.fileId,
                    chunkIds
              ...
              console.log(`📊 Usando PostgreSQL para embeddings (método original)`);
            ...
            // Mejorar mensaje de error para problemas de API key
            ...
            throw {
              message: isApiKeyError
                ? `Error de API Key para embeddings (${provider}/${model}). Verifica la configuración de OpenAI en el whitelabel. Detalle: ${errorMsg}`
                : `Error al generar embeddings: ${errorMsg}`,
