...
describe('api-ia service (esqueleto)', () => {
  ...
  it('con flag=true → sendChatMessage hace POST a /api/messages/send con JWT + X-Development + body correcto', async () => {
    ...
    await api.sendChatMessage('conv-123', 'hola mundo');

    const [url, opts] = fetchMock.mock.calls[0];
    expect(String(url)).toContain('/api/messages/send');
    expect(opts.method).toBe('POST');
    expect(opts.headers.Authorization).toBe('Bearer test.jwt.token');
    expect(opts.headers['X-Development']).toBe('bodasdehoy');
    const body = JSON.parse(opts.body);
    expect(body).toMatchObject({ channel: 'LOBE_CHAT', conversationId: 'conv-123', text: 'hola mundo' });
  });

  it('con flag=true → getChatMessages hace GET a /api/messages/conversations/{id}/messages', async () => {
    vi.stubEnv('NEXT_PUBLIC_USE_API_IA_ENDPOINTS', 'true');
    vi.resetModules();
    ...
    const res = await api.getChatMessages('conv-123', { limit: 50 });

    const [url, opts] = fetchMock.mock.calls[0];
    expect(String(url)).toContain('/api/messages/conversations/conv-123/messages');
