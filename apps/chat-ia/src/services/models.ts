...
const isEnableFetchOnClient = (provider: string) => {
  // ✅ IMPORTANTE: Si el provider es "auto" o "auto-max" y se usa backend Python,
  // NO usar fetchOnClient (que inicializa ModelRuntime en el cliente y valida API keys)
  if (provider === 'auto' || provider === 'auto-max') {
    try {
      const { USE_PYTHON_BACKEND, PYTHON_BACKEND_URL } = getPythonBackendConfig();

      if (USE_PYTHON_BACKEND && PYTHON_BACKEND_URL) {
        console.log(`✅ Provider "${provider}" con backend Python - forzando request al servidor (evita validación de API keys en cliente)`);
