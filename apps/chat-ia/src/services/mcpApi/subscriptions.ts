/**
 * Servicio de Planes de Suscripción — MCP GraphQL
 * =================================================
...
export interface ProductLimit {
  daily_quota?: number;
  free_quota: number;
  /** Max requests/hora — configurable por plan en MCP, leído por api-ia para velocity throttle */
...
const GET_SUBSCRIPTION_PLANS_QUERY_FREE_QUOTA = `
...
const GET_SUBSCRIPTION_PLANS_QUERY_DAILY_QUOTA = `
  query GetSubscriptionPlans($development: String!, $tier: SubscriptionTier, $is_public: Boolean) {
    getSubscriptionPlans(development: $development, tier: $tier, is_public: $is_public) {
      _id
      plan_id
...
export async function getMySubscription(): Promise<UserSubscriptionInfo | null> {
  try {
    const data = await mcpClient.query<{ getMySubscription: UserSubscriptionInfo | null }>(
...
export async function subscribeToPlan(
  ...
  try {
    ...
    const data = await mcpClient.query<{ subscribeToPlan: SubscribePlanResult }>(
...
export async function cancelSubscription(): Promise<CancelSubscriptionResult> {
  try {
    // MCP real mutation name: cancelMySubscription → returns UserSubscription
    const data = await mcpClient.query<{ cancelMySubscription: { _id: string; plan_id: string, status: string; } | null }>(
...
export async function createCustomerPortalSession(returnUrl?: string): Promise<CustomerPortalResult> {
  try {
    const origin = typeof window !== 'undefined' ? window.location.origin : '';
    const data = await mcpClient.query<{ createCustomerPortalSession: CustomerPortalResult }>(
...
export async function getSubscriptionPlans(
  ...
  try {
    ...
    const queries = [GET_SUBSCRIPTION_PLANS_QUERY_FREE_QUOTA, GET_SUBSCRIPTION_PLANS_QUERY_DAILY_QUOTA];

    let lastError: unknown;
    for (const query of queries) {
      try {
        const data = await mcpClient.query<{ getSubscriptionPlans: SubscriptionPlan[] }>(query, variables);
    ...
    console.error('[subscriptions] Error obteniendo planes:', lastError);
    return [];
  } catch (error) {
