...
export class WalletService {
  ...
  async getBalance(): Promise<WalletBalance> {
    try {
      ...
      const data = await mcpClient.query<{ wallet_getBalance: WalletBalance }>(GET_BALANCE_QUERY);
  ...
  async checkBalance(amount: number): Promise<BalanceCheck> {
    try {
      const data = await mcpClient.query<{ wallet_checkBalance: BalanceCheck }>(CHECK_BALANCE_QUERY, {
        amount,
      });
  ...
  async getServicePrice(sku: string, quantity: number = 1): Promise<ServicePrice> {
    try {
      const data = await mcpClient.query<{ wallet_getServicePrice: ServicePrice }>(
  ...
  async getTransactions(
    ...
    try {
      ...
      const data = await mcpClient.query<{ wallet_getTransactions: TransactionsResponse }>(
  ...
  async createRechargeSession(
    ...
    try {
      ...
      const data = await mcpClient.query<{ wallet_createRechargeSession: RechargeSessionResponse }>(
  ...
  async getPaymentMethods(): Promise<StoredPaymentMethod[]> {
    try {
      const data = await mcpClient.query<{ wallet_getPaymentMethods: StoredPaymentMethod[] }>(
  ...
  async getAutoRechargeConfig(): Promise<WalletAutoRechargeConfig | null> {
    try {
      const data = await mcpClient.query<{ wallet_getAutoRechargeConfig: WalletAutoRechargeConfig | null }>(
  ...
  async configureAutoRecharge(
    ...
    try {
      ...
      const data = await mcpClient.query<{ wallet_configureAutoRecharge: ConfigureAutoRechargeResponse }>(
  ...
  async checkAndConsume(
    ...
    try {
      ...
      const data = await mcpClient.query<{ wallet_checkAndConsume: ConsumeResponse }>(
