...
export class ClientService extends BaseClientService implements ISessionService {
  ...
  getSessionConfig: ISessionService['getSessionConfig'] = async (id) => {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Timeout: getSessionConfig tardó más de 3 segundos')), 3000);
    });
    ...
    try {
      ...
      console.error(`❌ [SessionConfig] Error obteniendo configuración de ${id}:`, error);
      throw error;
    }
  };
