...
// Si getLLMConfig() falla, retornamos un objeto con valores por defecto en desarrollo
let llmEnv: ReturnType<typeof getLLMConfig>;
try {
  llmEnv = getLLMConfig();
} catch (error: any) {
  // En desarrollo, usar valores por defecto en lugar de fallar
  if (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV) {
    console.warn('⚠️ Error al cargar llmEnv, usando valores por defecto:', error.message);
