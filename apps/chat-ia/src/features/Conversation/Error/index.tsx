...
export const useErrorContent = (error: any) => {
  ...
  return useMemo<AlertProps | undefined>(() => {
    ...
    // ✅ CORRECCIÓN: Asegurar que providerName tenga un valor por defecto
    const finalProviderName = providerName || error?.body?.provider || 'API';
    
    // ✅ CORRECCIÓN: Reemplazar manualmente {{provider}} si i18next no lo hace
    let message = t(`response.${messageError.type}` as any, { provider: finalProviderName });
    
    // ✅ Si el mensaje aún contiene {{provider}}, reemplazarlo manualmente
    if (message && message.includes('{{provider}}')) {
      message = message.replaceAll('{{provider}}', finalProviderName);
    }
