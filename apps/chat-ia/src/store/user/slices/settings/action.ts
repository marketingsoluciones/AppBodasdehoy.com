    ...
    if (settingsParams) {
      try {
        ...
        if (importSettings?.message || !importSettings?.data) {
          // handle some error
          console.warn('⚠️ [importUrlShareSettings] Error decodificando settings:', importSettings?.message);
          return;
        }

        await get().setSettings(importSettings.data);
      } catch (error) {
        console.error('❌ [importUrlShareSettings] Error parseando settings desde URL:', error);
        // Limpiar parámetro corrupto de la URL para evitar errores futuros
