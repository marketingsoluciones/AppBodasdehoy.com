...
> = (set, get) => ({
  async createImage() {
    ...
    try {
      ...
      // 6. Update store with the generated batch
      const isWhitelabelMode =
        typeof window !== 'undefined' && !!localStorage.getItem('dev-user-config');

      if (isWhitelabelMode && result.success && result.data?.batch) {
        // Whitelabel: images generated by Python backend, not saved to DB.
        // Add the batch directly to the store so the UI shows the results.
        const batch = result.data.batch as any;
        if (batch.generations?.length > 0) {
          set(
            {
              generationBatchesMap: {
                ...get().generationBatchesMap,
                [finalTopicId!]: [batch, ...(get().generationBatchesMap[finalTopicId!] || [])],
              },
            },
            false,
            'createImage/setWhitelabelBatch',
          );
        }
      } else {
        // Standard mode: batches are saved by the lambda, refresh from DB.
