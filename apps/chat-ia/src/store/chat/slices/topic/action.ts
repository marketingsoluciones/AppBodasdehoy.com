...
/**
 * ✅ GUEST MODE: Helper para detectar si el usuario actual es guest
 * Evita escrituras en la DB de PostgreSQL para usuarios no autenticados
 */
...
> = (set, get) => ({
  ...
  removeSessionTopics: async () => {
    ...
    // ✅ GUEST MODE: Solo borrar de memoria local para guests
    if (!isGuestUser()) {
  ...
  removeGroupTopics: async (groupId: string) => {
    ...
    // ✅ GUEST MODE: Solo borrar de DB si no es guest
    if (topicIds.length > 0 && !isGuestUser()) {
  ...
  internal_updateTopic: async (id, data) => {
    ...
    // ✅ GUEST MODE: Solo actualizar en DB si no es guest
    if (!isGuestUser()) {
  ...
  internal_createTopic: async (params) => {
    ...
    let topicId: string;
    if (isGuestUser()) {
      // Para usuarios guest, usar ID temporal sin persistir en DB
      console.log('👤 Guest mode: Topic creado solo en memoria (no se persiste en DB)');
      topicId = `guest_topic_${tmpId}`;
      get().internal_updateTopicLoading(tmpId, false);
      // Actualizar el topic temporal con el ID final
      get().internal_dispatchTopic(
        { type: 'updateTopic', id: tmpId, value: { id: topicId } },
        'internal_createTopic/guest',
      );
    } else {
      // Para usuarios autenticados, persistir en DB
      topicId = await topicService.createTopic(params);
      get().internal_updateTopicLoading(tmpId, false);
