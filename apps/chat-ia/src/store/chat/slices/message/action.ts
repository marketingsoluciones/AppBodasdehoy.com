...
const normalizeDeliveryStatus = (messages: ChatMessage[] = []) =>
  messages.map((message) => {
    if (message.role !== 'user' || message.deliveryStatus) return message;
    return { ...message, deliveryStatus: 'synced' as DeliveryStatus };
  });
...
export interface ChatMessageAction {
  ...
  /**
   * Update the active session type cache
   */
  internal_updateActiveSessionType: (type?: 'agent' | 'group') => void;
}
...
> = (set, get) => ({
  ...
  useFetchMessages: (enable, sessionId, activeTopicId) =>
    useClientDataSWR<ChatMessage[]>(
      ...
      // Migración Opción A: getMessages se resuelve en el service (messageService =
      // ApiIaMessageService cuando USE_API_IA_ENDPOINTS, ver services/message/index.ts).
      // UN solo camino — no duplicar el flag aquí.
        ...
        messageService.getMessages(sessionId, topicId),
      {
        onSuccess: (messages, key) => {
          ...
          set(
            ...
            false,
            n('useFetchMessages', { messages: normalizedMessages, queryKey: key }),
          );
        },
      },
    ),
    ...
    set(
      {
        messagesMap: {
          ...get().messagesMap,
          [messageMapKey(get().activeId, get().activeTopicId)]: normalizedMessages,
        },
      },
      false,
      'replaceMessages',
  ...
  internal_createMessage: async (message, context) => {
    ...
    try {
      ...
      internal_dispatchMessage({
        ...
        type: 'updateMessage',
        value: {
          deliveryStatus: isUserMessage ? 'error' : undefined,
  ...
  internal_fetchMessages: async () => {
    ...
    const nextMap = {
      ...get().messagesMap,
      [chatSelectors.currentChatKey(get())]: normalizedMessages,
    };
    // no need to update map if the messages have been init and the map is the same
    if (get().messagesInit && isEqual(nextMap, get().messagesMap)) return;
  ...
  internal_createTmpMessage: (message) => {
    ...
    const deliveryStatus =
      ...
      (message.role === 'user' ? ('pending' as DeliveryStatus) : undefined);
    internal_dispatchMessage({
      type: 'createMessage',
      id: tempId,
      value: { ...message, deliveryStatus },
    });
  ...
  internal_updateActiveSessionType: (type) => {
    set({ activeSessionType: type }, false, n('updateActiveSessionType'));
  },
});
