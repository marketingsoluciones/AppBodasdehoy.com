...
const onMouseDown = (e, item) => {
  console.log("DOWN")
  const child = document.getElementById(`icon${item.title}`)
  const rootElement = document.getElementById('areaDrag');
  const element = document.createElement('div');
  element.id = `dragM${item.title}`
...
const onUp = (e, item: table) => {
  console.log("UP")
  const rootElement = document.getElementById('areaDrag');
  const element = document.getElementById(`dragM${item.title}`)
  element && rootElement.removeChild(document.getElementById(`dragM${item.title}`))
}
...
const DragTable: FC<propsDragTable> = ({ item, set, state, setModelo }) => {
  ...
  return (
    <div className="static">
      <div id={`icon${item.title}`} className="hidden">
      ...
      >
        <div className="w-full h-full p-2 flex-col justify-center items-center cursor-pointer relative">
          <div className="w-full h-full flex transform hover:scale-105 transition justify-center items-center relative">
            <div id={`dragN${item.title}`} className="js-dragTable"
