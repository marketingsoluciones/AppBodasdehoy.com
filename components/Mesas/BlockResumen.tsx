...
const BlockResumen: FC <propsBlockResumen> = ({ InvitadoSentados }) => {
    ...
    return (
        <div className="bg-primary w-full px-6 pb-4 rounded-lg">
            <h2 className="text-tertiary font-display text-medium text-lg py-3">Resumen</h2>
            <div className="flex flex-col gap-8">
                {Datos.map((item,idx) => (
                <div key={idx} className="flex w-max gap-3 items-center justify-center">
                    <MesaIcon className="text-white h-5 w-auto"/>
                    <p className="text-white font-display font-semibold text-lg leading-4">{item.title}  <span className="text-xs font-light">{item.subtitle}</span></p>
                </div>
