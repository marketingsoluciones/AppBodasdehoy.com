...
const BlockDefault: FC<any> = ({ children, listaLength }) => {
  ...
  const handleResize = () => {
    ...
    if (window.innerWidth < 1536) {
      ...
      if (r == 1) setClassNameGrid("h-[50%] grid-rows-1 grid-cols-4")
      if (r == 2) setClassNameGrid("h-[100%] grid-rows-2 grid-cols-4")
      if (r == 3) setClassNameGrid("h-[150%] grid-rows-3 grid-cols-4")
      if (r == 4) setClassNameGrid("h-[200%] grid-rows-4 grid-cols-4")
      if (r == 5) setClassNameGrid("h-[250%] grid-rows-5 grid-cols-4")
    }
  ...
  return (
    <div className="w-full h-full overflow-auto text-sm">
      <div className={`w-full truncate grid ${classNameGrid}`} >
        {children}
      </div>
    </div>
  );
