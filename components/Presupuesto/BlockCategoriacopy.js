...
const BlockCategoriaCopy = ({ cate, set, setGetId }) => {
  ...
  const [expandedClick, setExpandedClick] = useState("")

  console.log(expandedClick)
        ...
        Cell: (props) => {
          ...
          return (
            <div className="w-full h-full flex items-center justify-center cursor-pointer relative space-x-1">
              ...
              <DetallesPago {...props} set={act => setGastoID(act)} setExpandedClick={setExpandedClick} />
              <BorrarIcon
  ...
  const renderRowSubComponent = useCallback(({ row, cate, gasto }) => {
    if (expandedClick == "RowClick") {
      return (
        <SubComponenteItemsTable row={row} cate={cate} gasto={gasto} />
      )
    }

    if (expandedClick == "pagosClick") {
      return (
  ...
  return (
    <>
      ...
      <div className={`bg-white block-categoria h-max py-10 w-full rounded-xl shadow-lg overflow-hidden flex flex-col items-center relative ${GastoID.crear ? "hidden" : "block"}`}>
        ...
        {/* Tabla de datos */}
        <DataTable setExpandedClick={setExpandedClick} AddGasto={AddGasto} columns={Columna} data={data ?? []} renderRowSubComponent={renderRowSubComponent} cate={categoria._id} gasto={GastoID.id} categoria={categoria} />
...
export default BlockCategoriaCopy;

export const DataTable = ({ data, columns, AddGasto, renderRowSubComponent, cate, gasto, categoria, setExpandedClick }) => {
  const { t } = useTranslation();
          ...
          return (
            <>
              <tr
                ...
                {...row.getRowProps()}
                className="w-full transition border-b border-base hover:bg-base grid grid-cols-11 relative"
              >
                {row.cells.map((cell, i) => {
                  return (
                    <td
                      key={i}
                      {...cell.getCellProps()}
                      className={`font-display text-sm w-full flex justify-center py-2 col-span-${colSpan[cell.column.id]
                        }`}
                    >
                      <div className="w-max">
                        {cell.render("Cell")}
                ...
                <div className="absolute left-5 top-3 cursor-pointer " onClick={() => { handleRowExpanded(row) }} >
                  <GoChevronDown className={`h-5 w-5 ${row.isExpanded ? "rotate-180 transition-all " : "transition-all"}`} />
                </div>
              </tr>
              {row.isExpanded ? (
                <tr key={i} className=" w-full border-b border-base">
...
export const SubComponenteItemsTable = ({ row, cate, gasto }) => {
  const [data, setData] = useState([]);
  ...
  const { headerGroups, prepareRow, rows } = useTable({ columns, data }, useExpanded);
  return (
    <table
      className="table w-full  relative rounded-t-md "
    >
      <thead className="">
        {headerGroups.map((headerGroup, id) => (
          <tr
            {...headerGroup.getHeaderGroupProps()}
            className="w-full grid grid-cols-9 bg-base py-2 rounded-t-md border-x border-t  "
            key={id}
          >
            {headerGroup.headers.map((column, id) => (
              <th
      ...
      <tbody className="text-gray-500 text-sm">
        {rows.map((row, i) => {
          ...
          return (
            <tr
              key={i}
              {...row.getRowProps()}
              className=" transition hover:bg-base grid grid-cols-9 border-x- bg-red "
            >
              {row.cells.map((cell, i) => {
                return (
                  <td
        ...
        <tr className="w-full transition border-b border-base  cursor-pointer  grid grid-cols-9 border-x">
          <td
            onClick={() => !isAllowed() ? ht() : AddGasto()}
            className="col-span-7 font-display capitalize text-sm text-primary w-full text-left py-3 flex gap-2  pl-3"
          >
            <div className="flex items-center gap-2 hover:opacity-90 hover:translate-x-2 transition transform">
          ...
          <td
            onClick={() => !isAllowed() ? ht() : AddGasto()}
            className="col-span-2 font-display capitalize text-sm w-full  py-3 flex gap-2 text-center  "
          >
            <div>
              Total:
            </div>
