    ...
    return (
        <>
            <div className=" w-full pt-6 relative rounded-lg pb-10">
                ...
                <div className=" bg-white rounded-b-lg max-h-[calc(100vh-320px)] h-[calc(100vh)] overflow-y-auto">
                    {
                        dataFactura.results.length ?
                            dataFactura?.total > 0 ?
                                dataFactura?.results?.map((item, idx) => {
                                    return (
                                        <div key={idx} className={`grid grid-cols-12 px-5 justify-between py-4 transition gap-y-4 text-gray-600 ${idx % 2 === 0 ? "bg-gray-50" : ""}`}>
                                            <div className="items-center col-span-3 flex flex-col ">
                                                {item.number}
                                            </div>
                                            ...
                                            <div className="col-span-3 flex h-full justify-center items-center space-x-1">
                                                <span>
                                                    {item.amount / 100}
                                                </span>
                                                <span>
                                                ...
                                                </span>
                                                {
                                                    ...
                                                    <ClickAwayListener onClickAway={() => show && setShow({ state: false, idx })}>
                                                        <div className="w-full flex justify-center items-center relative">
                                                            <ul
                                                                className={`${show.state === true ? "block" : "hidden"} top-0.5 right-16 absolute w-max border border-base bg-white capitalize rounded-md  shadow-lg z-10 translate-x-[-12px] p-3 cursor-pointer hover:bg-gray-100`}
                                                            >
                                                                Descargar Factura
                                                            </ul>
                                                        </div>
