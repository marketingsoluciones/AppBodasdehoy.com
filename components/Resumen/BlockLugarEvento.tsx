...
export const BlockLugarEvento = () => {
    ...
    return (
        <div className="relative">
            <div onClick={() => isAllowed() && setOpenList(!openList)} className="bg-primaryOrg space-x-3 rounded-lg text-white flex justify-between  items-center pl-1.5  md:pl-5 shadow-lg font-display text-xl select-none h-full   ">
                <span>{t("eventvenue")}</span>
                <div className={`bg-white w-[45.8%] md:w-[62.54%] rounded-lg h-[40px] flex items-center justify-between relative px-2 ${isAllowed() && "cursor-pointer"}`}>
                    <label className="flex-1 font-display font-base text-sm text-gray-700 leading-3">{lugarEvento?.title}</label>
