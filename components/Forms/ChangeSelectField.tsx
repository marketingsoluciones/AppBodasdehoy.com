...
const ChangeSelectField: FC<propsSelectField> = ({ label, children, options, colSpan, ...props }) => {
    ...
    return (
        <>
            <div className={`w-max h-full flex items-center space-x-2  `}>
                <div className="font-display text-sm text-primary hidden md:block ">{label}</div>
                <div>
                    <select
                        className="font-display capitalize cursor-pointer text-sm text-gray-500 border border-gray-300 focus:border-primary transition w-full py-2 mr-2 rounded-xl focus:outline-none  " {...field} {...props} >
...
export default ChangeSelectField
