...
const BlockNotification = ({ state, set, evento }) => {
  ...
  return (
    <ClickAwayListener onClickAway={() => state && set(!state)}>
      <div
        className={`w-96 z-50 bg-white fixed top-0 right-0 h-screen flex flex-col items-center font-display py-12 transform transition duration-200  rounded-l-2xl shadow-lg  ${state ? initial : "translate-x-full"
          } `}
      >
        {/* Cabecera */}
        ...
        <div className="grid gap-2 min-w-full	px-6 ">
          {evento?.notificaciones_array?.length <= 0
            ? <h3 className="text-sm text-gray-500 py-4">{t("nonotifications")}</h3>
            : (
              evento?.notificaciones_array?.map((item, idx) => (
