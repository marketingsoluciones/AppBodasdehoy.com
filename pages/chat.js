...
import GrupoEventosContext from '../context/GrupoEventos'
  ...
  <div>                <button className="button-primary">
                ...
                <div>
                    ...
                    {GrupoEventos.map( (linea,i) => (
                        <div key={i}>
                            <button className="text-black bg-gray-300 px-4 py-2 hover:bg-gray-900 hover:text-white transition" onClick={e=> cargar_chatsygrupos(e, linea)} type="button">{`${linea.nombre}`}</button>
                        </div>
                    )
                    )}
                </div>
